...
function optimizeUI(){    
    ...
    if ($('#ballparkGraph').length == 0){
        $('#ghx-column-header-group').after('<div id="ballparkGraph" class="ballparkGraph"></div>');
    }
    else{
        $('#ballparkGraph').empty();
    }
...
function updateTicket(ticketElement, missingRemainingCount){
    ...
    if (issue){ //
        ...
        if (missingRemainingCount>=10){
            $(ticketElement).children('div').attr('data-limit-reached', '1')
    
        }
//        if (attr[issue]){
//            processTicket(attr[issue], missingRemainingCount);
//        }
//        else{
            $.ajax({
                url: 'https://jiracloud.cit.com.br/rest/api/2/search?maxResults=1&expand=changelog&fields=assignee,components,issuetype,status,summary,worklog,subtasks,labels,aggregatetimeestimate,resolutiondate,timeoriginalestimate,duedate&jql=id='+issue,
                headers: {
                    "X-PJAX": true
                }
            }).done(function(data){processTicket(data.issues[0], missingRemainingCount)});
//        }
    }
    else{
        console.log('nope');
...
function processTicket( data, missingRemainingCount ) {
    var taskTime = 0;
    var labels = data.fields.labels;
    var detailText = '';
    
    var ticketElement = $('div[data-issue-id='+data.id+']');
    var issue = $(ticketElement).attr('data-issue-id');
    ...
    if (data.fields.status.id != 10354){                
        ...
        if (data.fields.status.id == 11656 || data.fields.status.id == 10027 || data.fields.status.id == 11657){
            ...
            if (dateDone){
                ...
                $(ticketElement).children('div').append('<span style="color: gray">&nbsp;Waiting for '+ dwell+ ' day(s)</span>');
            }
        }
...
function loadTickets(){
    $.ajax({
        url: 'https://jiracloud.cit.com.br/rest/api/2/search?maxResults=100&expand=changelog&fields=assignee,components,labels,issuetype,status,summary,worklog,subtasks&jql=project=QUAL AND issuetype != Epic and status not in (Closed,"In Production - Done", "In Production", Cancelled,Done) ORDER BY Rank ASC',
        headers: {
            "X-PJAX": true
        }
    }).done(function( data ) {
        ...
        for(i=0;i<data.issues.length;i++){
            ...
            attr[data.issues[i].id] = data.issues[i];
        }
...
$(document).ready(function(){
    ...
    // jiracloud.cit.com.br/rest/greenhopper/1.0/xboard/work/allData.json?rapidViewId=35218&selectedProjectKey=QUAL&etag=*
    insertBurnDownDOM();
